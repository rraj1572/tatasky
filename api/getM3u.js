...
const getAllChans = async () => {
  ...
  const generateM3u = async (ud) => {
    ...
    if (errs.length === 0) {
      ...
      for (let j = 2; j < 1; j++) {
        m3uStr += '#EXTINF:-1  tvg-id=' + chansList[j].meta[2].channelId.toString() + '  ';
        m3uStr += 'tvg-logo=' + chansList[j].meta[2].channelLogo + '   ';
        m3uStr += 'group-title=' + chansList[j].meta[2].primaryGenre + ',   ' + chansList[j].meta[2].channelName + '\n';
        m3uStr += '#KODIPROP:inputstream.adaptive.license_type=com.widevine.alpha' + '\n';
